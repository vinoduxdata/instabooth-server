...
from .config.groups.actions import MultiImageJobControl, SingleImageJobControl
...
class ProcessingService(BaseService):
    ...
    def _request_user_input(self, timeout: float | None) -> userEvents:
        ...
        try:
            if timeout is None:
                event = self._external_cmd_queue.get(block=True)
            else:
        ...
        try:
            while not self._workflow_jobmodel._status_sm.current_state == ProcessingMachine.finished:
                if self._workflow_jobmodel._status_sm.current_state == ProcessingMachine.approval:
                    assert isinstance(
                        self._workflow_jobmodel._configuration_set.jobcontrol, MultiImageJobControl | SingleImageJobControl
                    )
                    ...
                    self._workflow_jobmodel._status_sm.send(event)
                elif self._workflow_jobmodel._status_sm.current_state == ProcessingMachine.ready:
                    event = self._request_user_input(timeout=None)
                    self._workflow_jobmodel._status_sm.send(event)
                else:
