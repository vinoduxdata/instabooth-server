...
class SingleImageJobControl(BaseModel):
    ...
    ask_approval_each_capture: bool = Field(
        default=False,
        description="Stop after capture to review the shot and allow retake or print.",
    )
    approve_autoconfirm_timeout: float = Field(
        default=15.0,
        description="If approval is required, auto-accept after this many seconds (use a large value to disable).",
    )
