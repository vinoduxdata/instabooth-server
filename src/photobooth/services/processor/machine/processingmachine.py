...
class ProcessingMachine(StateMachine):
    ...
    ready = State()  # wait for user to touch before countdown
    ...
    next = Event(
        # first next is from start (init state) to ready (touch to capture)
        start.to(ready)
        # user confirms on ready screen, then countdown starts
        | ready.to(counting)
        # next after countdown is always to capture
        | counting.to(capture)
    ...
    abort = Event(finished.from_(start, ready, counting, capture, approval, completed, present))
