...
class JobModelBase(ABC, Generic[T]):
    ...
    def on_enter_ready(self):
        """Prepare acquisition before user confirms capture (touch to capture)."""
    ...
    def sm_cond_ask_user_for_approval(self) -> bool:
        jobcontrol = self._configuration_set.jobcontrol
        if isinstance(jobcontrol, MultiImageJobControl | SingleImageJobControl):
            return jobcontrol.ask_approval_each_capture
        return False
