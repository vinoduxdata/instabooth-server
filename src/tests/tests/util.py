...
def continue_touch_to_capture(processing_service=None):
    """Confirm the touch-to-capture screen to start countdown."""
    from photobooth.container import container

    svc = processing_service or container.processing_service
    if not svc._external_cmd_required.wait(timeout=5):
        raise RuntimeError("waiting for touch-to-capture not within timeout")
    svc.continue_process()
